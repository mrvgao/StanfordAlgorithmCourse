...
def random_select_vertex(graph, num=1, seed=None):
...
def random_contract_one_pass(graph: Graph, seed=None):
    while len(graph.get_vertices()) > 2:
        random_vertice_1, random_vertice_2 = random_select_vertex(graph, num=2, seed=seed)
...
def random_contract(g: Graph, verboes=False):
    ...
    for i in range(run_times):
        tmp_g = random_contract_one_pass(original_g, seed=i)
